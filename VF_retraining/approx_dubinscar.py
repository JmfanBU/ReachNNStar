...
if __name__ == '__main__':
    ...
    with U.make_session() as sess:
        ...
        input_range_low = [-5.315, -5.591, -2 * np.pi]
        input_range_high = [5.142, 1, 2 * np.pi]
        ...
        while iters < max_it or current_loss > args.regression_bound:
            ...
            x_in_1 = np.random.uniform([-5.315, -5.591, -np.pi], [-2.236, 1, np.pi], [40000, args.input_dim])
            x_in_2 = np.random.uniform([-2.236, -5.591, -np.pi], [3.030, -2.374, np.pi], [34000, args.input_dim])
            x_in_3 = np.random.uniform([3.030, -5.591, -np.pi], [5.142, 1, np.pi], [26000, args.input_dim])
            x_in = np.vstack((x_in_1, x_in_2, x_in_3))
            # and train on it
