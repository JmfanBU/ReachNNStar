...
int main()
{
	...
	char const *neural_network = "nn_4_relu_tanh";
	...
	string reach_result;
	reach_result = "Verification result: Unknown(5)";
	result.fp_end_of_time.intEval(end_box, order, setting.tm_setting.cutoff_threshold);

	if(end_box[0].inf() >= -0.05 && end_box[0].sup() <= 0.05 && end_box[1].inf() >= -0.05 && end_box[1].sup() <= 0){
		reach_result = "Verification result: Yes(5)";
	}

	if(end_box[0].inf() >= 0.05 || end_box[0].sup() <= -0.05 || end_box[1].inf() >= 0 || end_box[1].sup() <= -0.05){
		reach_result = "Verification result: No(5)";
	}
	...
	ofstream result_output("./outputs/nn_4_relu_tanh.txt");
	...
	plot_setting.plot_2D_interval_GNUPLOT("nn_4_relu_tanh", result);
